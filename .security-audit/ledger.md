...
| A6-HIGH-1 | High | Hubitat's cloud URL exposes the rule endpoints to the internet behind a `Host` check that is a no-op if the relay rewrites the header | **Closed 2026-08-05, verified on hardware.** A `/pool` call over `cloud.hubitat.com` to hub `303` returned the app's own 409 refusal, so the relay presents a `Host` the check rejects. Note what this does and does not establish: the cloud path still reaches the app, and the check is the only thing stopping it. Re-test after any hub firmware update, and treat `refuseCloudRequests` as a load-bearing setting rather than a preference. |
